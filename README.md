# QUANTUM GAUNTLET
[![Game Version](https://img.shields.io/badge/Latest-0.3.0-333.svg?style=flat)](https://unity3d.com)
[![Game Status](https://img.shields.io/badge/Status-BETA-f60.svg?style=flat)](https://unity3d.com)
[![Unity Version](https://img.shields.io/badge/UNITY-%202021.3.45-fff.svg?style=flat&logo=unity)](https://unity3d.com)
[![License](https://img.shields.io/badge/License-MIT-0298c3.svg?style=flat)](https://opensource.org/license/mit)

Quantum Gauntlet is 3D isometric local multiplayer arena brawler. It supports up to four players playing in Free For All or teams.


![image](https://github.com/user-attachments/assets/dc7da094-8a14-4391-be6d-1fa5b10c9cdb)



# Gameplay
Features a dynamic combat system with mana resources, combos, and power-ups. Contains a custom physics engine for juggling your opponents through the air. Models, animations, textures, shaders, sfx & music - all done from scratch! Supports gamepad and keyboard with an in-game tutorial to go over the basic controls.

<img src="https://github.com/user-attachments/assets/f90c1af3-0fca-4b40-8fc3-e09b83543ec6" width=50%><img src="https://github.com/user-attachments/assets/3eada87f-53d2-4ff9-aa5a-bfb4183c32cd" width=50%>

<a href="https://www.youtube.com/watch?v=0U-hM4oxC_U">Link to full gameplay trailer </a>

# Getting Started
1. Install Unity 2021.3.45
2. Export to desired runtime
3. Run appropriate executable in QUANTUMGAUNTLET directory.
4. Play!

# Contributors
<a href="https://github.com/Ebinsoft/QUANTUM_GAUNTLET/graphs/contributors">
<img src="https://contrib.rocks/image?repo=Ebinsoft/QUANTUM_GAUNTLET" />
</a>

# License
Licensed under the <a href="https://github.com/Ebinsoft/QUANTUM_GAUNTLET/blob/readme-update/LICENSE.txt"> MIT </a> license.